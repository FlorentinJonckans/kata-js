...
function stockList(listOfArt, listOfCat){
    ...
    const newListOfCat = listOfCat.map(c => {
      const stockListsCat = listOfArt.filter(sl => sl.indexOf(c) === 0)
      const booksInStockList = stockListsCat.map(sl => parseInt(sl.split(' ')[1]))
